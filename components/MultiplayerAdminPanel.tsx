...
import { ArrowLeft, Save, Trash2, Plus, Settings, Star, LayoutTemplateIcon as Template } from "lucide-react"
...
export function MultiplayerAdminPanel({ gameId, onBack }: MultiplayerAdminPanelProps) {
  ...
  const loadSpecialCells = async () => {
    const { data, error } = await supabase.from("special_cells").select("*").eq("game_id", gameId).order("position")
  ...
  const handleSaveCell = async () => {
    ...
    // Check for duplicate position (only if creating new cell or changing position)
    const existingCell = specialCells.find(
      (cell) => cell.position === editingCell.position && cell.id !== editingCell.id,
    )

    if (existingCell) {
      alert(`Ya existe una casilla especial en la posición ${editingCell.position}. Por favor elige otra posición.`)
      return
    }
    ...
    try {
      if (editingCell.id) {
        ...
        // Crear nueva casilla
        const { error } = await supabase.from("special_cells").insert({
          game_id: gameId,
          position: editingCell.position,
          name: editingCell.name,
          description: editingCell.description,
  ...
  const handleDeleteCell = async (cellId: string) => {
    ...
    try {
      const { error } = await supabase.from("special_cells").delete().eq("id", cellId)
  ...
  const startNewCell = () => {
    ...
    let nextPosition = 1
    const occupiedPositions = specialCells.map((cell) => cell.position).sort((a, b) => a - b)
  ...
  const handleLoadTemplate = async (templateCells: SpecialCell[]) => {
    setIsLoading(true)
    try {
      // Eliminar casillas especiales existentes
      const { error: deleteError } = await supabase.from("special_cells").delete().eq("game_id", gameId)

      if (deleteError) throw deleteError

      // Insertar nuevas casillas desde la plantilla
      if (templateCells.length > 0) {
        const cellsToInsert = templateCells.map((cell) => ({
          game_id: gameId,
        ...
        const { error: insertError } = await supabase.from("special_cells").insert(cellsToInsert)
    ...
    <div className="max-w-4xl mx-auto space-y-6">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-blue-700">Posición (1-62)</label>
                <Input
                  type="number"
                  min="1"
                  max="62"
                  ...
                  className={`border-blue-200 focus:border-blue-400 ${
                    specialCells.some((cell) => cell.position === editingCell.position && cell.id !== editingCell.id)
                      ? "border-red-300 bg-red-50"
                      : ""
                  }`}
                />
                {specialCells.some((cell) => cell.position === editingCell.position && cell.id !== editingCell.id) && (
                  <p className="text-xs text-red-600">⚠️ Ya existe una casilla especial en esta posición</p>
                )}
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-blue-700">Nombre de la casilla</label>
                <Input
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-blue-700">Descripción</label>
              <Input
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-blue-700">Efecto</label>
              <Select
      ...
      <Card className="bg-white/80 backdrop-blur-sm border-amber-200 shadow-xl">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-amber-800">Casillas Especiales ({specialCells.length})</CardTitle>
          <Button
        ...
        <CardContent>
          {specialCells.length === 0 ? (
            <div className="text-center py-8 text-amber-600">
              ...
              <p className="text-sm mb-4">Haz clic en "Nueva Casilla" para agregar efectos especiales al tablero</p>
              <Button
                onClick={() => setShowTemplateManager(true)}
                variant="outline"
                className="border-purple-200 text-purple-700 hover:bg-purple-50"
              >
                <Star className="w-4 h-4 mr-2" />O usa una plantilla predefinida
              </Button>
            </div>
          ) : (
